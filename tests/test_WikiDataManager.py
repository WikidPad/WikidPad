...
* Test renaming wiki words and updating references via
  WikiDataManager.renameWikiWords(renameDict, modifyText).
...
from tests.helper import MockWikiDataManager, get_text, TESTS_DIR
...
test_files = {
    'wikidpad_default_2_0':
        (u'test_wikis_wikidpad_default_2_0_before_rename.txt',
         u'test_wikis_wikidpad_default_2_0_after_rename.txt'),
}
...
def parse_test_wikis_text_file(s, wiki_language_name):
    ...
    def error_msg():
        return u'Parse error on line %d section %d: %r' % (
            line_nr, section, line)

    wiki_start = re.compile(ur'^= (?P<wikiname>\w+) =+$')
    wiki_end = re.compile(ur'^={80}$')
    page_start = re.compile(ur'^- (?P<pagename>[/\w]+) -+$')
    page_end = re.compile(ur'^-{80}$')
    rename_seq_line = lambda line: u'->' in line
    ...
    for line_nr, line in enumerate(s.splitlines(True), 1):
        if section == 0:  # outside wiki
            ...
            if rename_seq_line(line):
                for rename_couple in line.strip().split(u','):
                    old_page_name, new_page_name = [
                        s.strip() for s in rename_couple.split(u'->')]
            ...
            if m:
                ...
                wikidoc = MockWikiDataManager(wiki_content, wiki_language_name)
            ...
            if m:
                ...
                page_content = u''.join(page_lines)
...
def test_renameWikiWords():
    ...
    def err_msg():
        msg = u"Method %r language %r: updating text of %r failed"
    ...
    for method_name, modifyText in UPDATE_METHODS_TO_TEST:
        ...
        for language_name in test_files:
            tests = load_tests(language_name).iteritems()
