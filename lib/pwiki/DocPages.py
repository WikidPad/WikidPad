...
#                 return self.props
    ...
    def getProperty(self, propkey, default=None):
        with self.textOperationLock:
            ...
            if len(parents) > 0:
                ...
                for parent in parents:
                    try:
                        ...
                        templateWord = parentPage.getProperty(u"template")
                ...
                if templateSource is None and not conflict:
                    ...
                    templateWord = globalProps.get(u"global.template")
                    if templateWord is not None and \
                            self.wikiDocument.isDefinedWikiLink(templateWord):
                        templateSource = templateWord
...
# 
# 
#     def getFlatTree(self, unalias=False, includeSet=None, maxdepth=100,
#             includeRemain=True):
#         """
...
#         return result
    ...
    def getFlatTree(self, unalias=False, includeSet=None, level=-1):
        """
        Returns a sequence of tuples (word, deepness) where the current
        word is the first one with deepness 0.
        The words may contain aliases, but no word appears twice neither
        ...
        while len(checkList) > 0:
            word, nonAliasWord, chLevel = checkList.pop()
